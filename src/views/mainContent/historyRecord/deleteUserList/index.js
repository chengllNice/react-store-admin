...
import { getDeleteUserList, postResetDeleteUser, postPermanentDeleteUser} from "@/servers/historyRecordApi";
import { getUserBussinesRoleList} from "@/servers/commonApi";

@CBaseComponent
class HistoryRecordDeleteUserList extends Component{
  constructor(props) {
    super(props);
  ...
  getDeleteUserListData(){
    ...
    getDeleteUserList(getData).then(res=>{
  ...
  resetHandler(){
    ...
    this.refs.historyRecordDeleteUserList.clearFilterData();
  }

  // 恢复已删除用户
  resetRecord(data){
    postResetDeleteUser({id: data.id}).then(res=>{
      this.getDeleteUserListData();
    })
  }
  ...
  deletePermanent(data){
    ...
    Modal.confirm({
      ...
      content: '',
      onOk(){
        postPermanentDeleteUser(delData).then(res=>{
  ...
  componentDidMount(){
    // console.log(this.refs.historyRecordDeleteUserList.clearFilterData(),'refffffffffff')
    this.getDeleteUserListData();
    this.getUserBussinesRoleListData();
  }
  render(){
    return (
      <div className='history-record-delete-user-list'>
        ...
        <CTable ref='historyRecordDeleteUserList' checked={false} loading={this.state.loading} columns={this.state.columns} dataSource={this.state.dataSource} page={this.state.page} onTableChange={this.onTableChange}>
          <div
            slot='operate'
...
export default connect(mapStateToProps, mapDispatchToProps)(HistoryRecordDeleteUserList);
